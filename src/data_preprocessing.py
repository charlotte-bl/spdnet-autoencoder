...
def add_gaussian_noise_to_covariances(cov, std=1):
    ...
    for i in range(cov.shape[0]):
        ...
        cov_noisy = torch.cat(
            (cov_noisy, (cov[i][0] + noise).unsqueeze(0).unsqueeze(0)), dim=0
        )
    ...
    for i in range(cov.shape[0]):
        ...
        cov_noisy = torch.cat(
            (
                ...
                (
                    eigenvectors
                    @ torch.diag(eigenvalues.type(torch.cdouble))
...
class NoisyCleanLabeledDataset(torch.utils.data.Dataset):
    def __init__(
        self, data, labels, add_noise=add_gaussian_noise_to_covariances, std=1
    ):
        self.data = data
...
def raw_to_cov(raw_data):
    ...
    return torch.from_numpy(
        Covariances(estimator="scm").fit_transform(raw_data)
    ).unsqueeze(1)
...
def preprocess_data_cov(X, labels, batch_size, noise, std):
    ...
    if noise == "salt_pepper":
        ...
        dataset = NoisyCleanLabeledDataset(
            X, labels, add_masking_noise_to_covariances, std
        )
    elif noise == "gaussian":
        dataset = NoisyCleanLabeledDataset(
            X, labels, add_gaussian_noise_to_covariances, std
        )
    elif noise == "gaussian_eigen":
        dataset = NoisyCleanLabeledDataset(
            X, labels, add_gaussian_noise_eigen_decomposition, std
        )
    else:
        dataset = LabeledDataset(X, labels)

    x_train, x_val, x_test = torch.utils.data.random_split(
        dataset, lengths=[0.5, 0.25, 0.25]
    )
    ...
    test_loader = torch.utils.data.DataLoader(
        x_test, batch_size=batch_size * 2, shuffle=False
    )
    return train_loader, val_loader, test_loader
...
def preprocess_data_BCI(X, labels, batch_size, noise, std):
    data_train_val, labels_train, data_test, labels_test = train_test_split_BCI(
        X, labels
    )
    ...
    # add noise if needed
    if noise == "salt_pepper":
        dataset_train_val = NoisyCleanLabeledDataset(
            cov_train_val, labels_train, add_salt_and_pepper_noise_to_covariances, std
        )
        x_test = NoisyCleanLabeledDataset(
            cov_test, labels_test, add_salt_and_pepper_noise_to_covariances, std
        )
    elif noise == "masking":
        dataset_train_val = NoisyCleanLabeledDataset(
            cov_train_val, labels_train, add_masking_noise_to_covariances, std
        )
        x_test = NoisyCleanLabeledDataset(
            cov_test, labels_test, add_masking_noise_to_covariances, std
        )
    elif noise == "gaussian":
        dataset_train_val = NoisyCleanLabeledDataset(
            cov_train_val, labels_train, add_gaussian_noise_to_covariances, std
        )
        x_test = NoisyCleanLabeledDataset(
            cov_test, labels_test, add_gaussian_noise_to_covariances, std
        )
    elif noise == "gaussian_eigen":
        dataset_train_val = NoisyCleanLabeledDataset(
            cov_train_val, labels_train, add_gaussian_noise_eigen_decomposition, std
        )
        x_test = NoisyCleanLabeledDataset(
            cov_test, labels_test, add_gaussian_noise_eigen_decomposition, std
        )
    else:
    ...
    x_train, x_val = torch.utils.data.random_split(
        dataset_train_val, lengths=[0.8, 0.2]
    )
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader = load_preprocess_synthetic_data(
        1, "block_diag"
    )
