...
import config as c
from save import (
    find_second_folder_results_influence_of_encoding_dim_name,
    find_path,
    find_comparison_path,
)
...
def load_dict(path, name):
    ...
    reconstructed_dict = {
        k: (m, s) for k, m, s in zip(loaded_keys, loaded_means, loaded_stds)
    }
    return reconstructed_dict


def compare_channels_encoding(
    epochs,
    channels_out,
    losses,
    layers_type,
    ...
    folder_comparison = find_comparison_path(
        epochs, data, noise, std, batch_size, generation
    )
    ...
    second_folder_acc_init = find_second_folder_results_influence_of_encoding_dim_name(
        epochs,
        channels_out[0],
        losses[0],
        layers_type[0],
    ...
    for layer in layers_type:
        for channel in channels_out:
            for loss in losses:
                ...
                acc_decode = load_dict(
                    path, c.results_accuracy_decoding + c.extension_dict
                )
                list_dict_acc.append(
                    (
                        acc_decode,
                        f"Accuracy | channel : {channel} | layer : {layer} | loss : {loss} ",
                    )
                )

                trustworthiness = load_dict(
                    path, c.results_trustworthiness + c.extension_dict
                )
                list_dict_trustworthiness.append(
                    (
                        trustworthiness,
                        f"Trustworthiness | channel : {channel} | layer : {layer} | loss : {loss} ",
                    )
                )

                loss_value = load_dict(path, c.results_losses + c.extension_dict)
                dict_dict_losses[loss].append(
                    (
                        loss_value,
                        f"Loss | channel : {channel} | layer : {layer} | loss : {loss}",
                    )
                )

    # create images
    show_metrics_from_dicts(
        list_dict_acc,
        path=folder_comparison + "/acc",
        xlabel="Encoding dimension",
        ylabel="Accuracy",
        title="Accuracy in function of encoding dimension",
    )
    show_metrics_from_dicts(
        list_dict_trustworthiness,
        path=folder_comparison + "/trustworthiness",
        xlabel="Encoding dimension",
        ylabel="Trustworthiness",
        title="Trustworthiness in function of encoding dimension",
    )
    for loss in losses:
        show_metrics_from_dicts(
            dict_dict_losses[loss],
            path=folder_comparison + "/loss_" + loss,
            xlabel="Encoding dimension",
            ylabel="Loss",
            title="Loss in function of encoding dimension",
        )
