    ...
    for i in range(ho):
        for j in range(hi):
            if ni == no:
                ...
                ni < no
            ):  # augmentation de dimension pour retourner dans l'espace initial
                v = th.empty(no, no, dtype=W.dtype, device=W.device).uniform_(0.0, 1.0)
                u, s, v_t = th.svd(v.matmul(v.t()))
                W.data[i, j] = v_t.t()[:ni, :]
            else:  # cas no<ni - reduction de dimension pour aller dans l'espace latent
                v = th.empty(ni, ni, dtype=W.dtype, device=W.device).uniform_(0.0, 1.0)
...
def cov_pool_mu(f, reg_mode):
    ...
    aug2 = th.cat(
        (
            ...
            th.ones(mu.shape[0], 1, 1, dtype=mu.dtype, device=f.device),
        ),
        2,
    )
...
def dist_riemann(x, y):
    ...
    """
    return (
        LogEig.apply(CongrG(x, y, "neg"))
        .view(x.shape[0], x.shape[1], -1)
        .norm(p=2, dim=-1)
    )  # moyenne riemanniene si on minimise B par rapport a A_i
...
class Re_op:
    ...
    def fn_deriv(cls, S, param=None):
        return (
            S > cls._threshold
        ).float()  # use to be .double() but float64 not supported by MPS
