...
def find_second_folder_no_numero(
    epochs,
    encoding_dim,
    encoding_channel,
    loss,
    ...
    if data == "synthetic":
        second_folder += (
            f"{c.models_information_synthetic_generation}{synthetic_generation}"
        )
...
def find_second_folder_results_influence_of_encoding_dim_name(
    epochs,
    encoding_channel,
    loss,
    layers_type,
...
def find_second_folder_comparison(
    epochs, data, noise, std=None, batch_size=None, generation=None
):
...
def find_result_path(
    ...
    name = find_second_folder_results_influence_of_encoding_dim_name(
        epochs,
        encoding_channel,
        loss,
        layers_type,
...
def find_comparison_path(
    epochs, data, noise, std=None, batch_size=None, generation=None
):
    folder = c.comparison_folder
    name = find_second_folder_comparison(
...
def find_name_folder(
    ...
    second_folder = find_second_folder_no_numero(
        epochs,
        encoding_dim,
        encoding_channel,
        loss,
...
def save_model(model, folder):
    path = (
        folder + c.models_information_model_name + c.models_information_model_extension
    )
...
def save_all(
    ...
    show_first_image_from_loader(
        outputs_train, path, name="last_batch_reconstruction_train"
    )
    show_first_image_from_loader(data_val, path, name="last_batch_original_val")
    show_first_image_from_loader(
        outputs_val, path, name="last_batch_reconstruction_val"
    )
    show_first_image_from_loader(data_test, path, name="last_batch_original_test")
    show_first_image_from_loader(
        outputs_test, path, name="last_batch_reconstruction_test"
    )
    ...
    if trustworthiness_encoding is not None:
        save_datas_from_model(
            trustworthiness_encoding, path, name="trustworthiness_encoding"
        )
