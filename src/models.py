...
class Autoencoder_layers_byhalf_SPDnet(nn.Module):
    def __init__(self, ho, hi, ni, no):
        ...
        while current_size > self.no:
            current_size = max(
                self.no, int(current_size / 2)
            )  # moitié mais jamais en dessous de no
        ...
        for i in range(len(self.layer_sizes) - 1):
            encoder_layers.append(
                spdnet.BiMap(
                    self.channels_size[i + 1],
                    self.channels_size[i],
                    self.layer_sizes[i],
                    self.layer_sizes[i + 1],
                )
            )
...
class Autoencoder_hourglass_channel_SPDnet(nn.Module):
    def __init__(self, ho, hi, ni, no):
        ...
        self.encoder = nn.Sequential(
            spdnet.BiMap(
                self.ho, self.hi, self.ni, self.ni // 2
            ),  # on a une premiere couche qui va en filtre de 1 -> 3
            spdnet.ReEig(),
