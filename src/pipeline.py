...
def main():
    ...
    model = Autoencoder_SPDnet(ho, hi, ni, no)
    if args.loss == 'riemann':
        criterion = RiemannianDistanceLoss()
    else:
        criterion = torch.nn.MSELoss()
    data_train,outputs_train,list_train_loss,data_val,outputs_val,list_val_loss = train(train_loader,val_loader,model,args.epochs,criterion)

    #test model
    data_test,outputs_test,test_loss = test(test_loader,model,criterion)
