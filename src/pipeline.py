...
from models import (
    ...
    Autoencoder_layers_byhalf_SPDnet,
    Autoencoder_one_layer_SPDnet,
    Autoencoder_hourglass_channel_SPDnet,
)
...
def main():
    ...
    if args.data == "bci":
        ...
        train_loader, val_loader, test_loader = preprocess_data_BCI(
            X, labels, batch_size=args.batch_size, noise=args.noise, std=args.std
        )
        ...
        train_loader, val_loader, test_loader = load_preprocess_synthetic_data(
            args.index, args.synthetic_generation
        )
        ho, hi, ni, no = (
            args.encoding_channel,
            1,
            get_size_matrix_from_loader(train_loader),
            args.encoding_dim,
        )
    ...
    (
        ...
        list_val_loss,
        *optional_values,
    ) = train(train_loader, val_loader, auto_encoder, args.epochs, criterion)
    if noised:
        noisy_train, noisy_val = optional_values[0], optional_values[1]

    # test autoencoder
    data_test, outputs_test, test_loss, trustworthiness, *optional_values = test(
        test_loader, auto_encoder, criterion, show=args.show
    )
    if noised:
        noisy_test = optional_values[0]
    if auto_encoder.ho == 1:
        trustworthiness_encoding = optional_values[
            -1
        ]  # si il y a pas noise c'est 0 et si il y a c'est 1, c'est le dernier

    # accuracy of prediction
    acc_init, acc_decode, *optional_values = accuracy(
        auto_encoder, train_loader, val_loader, test_loader, args.loss
    )
    # if auto_encoder.ho == 1:
    #    acc_code = optional_values[0]

    # find folder name to save datas
    path = find_name_folder(
        "../models",
        args.epochs,
        args.encoding_dim,
        args.encoding_channel,
    ...
    save_all(
        ...
        trustworthiness_encoding=(
            trustworthiness_encoding if auto_encoder.ho == 1 else None
        ),
