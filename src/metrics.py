...
def pairwise_riemannian_distances(batch):
    ...
    for i in range(batch_size):
        for j in range(batch_size):
            dist_matrix[i, j] = dist_riemann_matrix(
                batch[i], batch[j]
            )  # distances entre matrice i du batch vs matrice j du batch
    return dist_matrix
...
def pairwise_euclidean_distances(batch):
    ...
    for i in range(batch_size):
        for j in range(batch_size):
            dist_matrix[i, j] = torch.norm(
                batch[i] - batch[j], p="fro"
            )  # distances entre matrice i du batch vs matrice j du batch
    return dist_matrix
...
def trustworthiness(
    original, reconstructed, k=2, pairwise_distance=pairwise_riemannian_distances
):
    batch_size = original.shape[0]
    orig_dist = diag_inf(
        pairwise_distance(original)
    )  # diag inf pour qu'une matrice n'ait pas elle meme comme plus proche voisin
    ...
    for i in range(batch_size):
        ...
        for j in range(batch_size):
            if (
                recon_ranks[i, j] < k
            ):  # pour que ce soit sur l'ensemble des k nearest neighbours dans l'espace output
...
def accuracy(auto_encoder, train_loader, val_loader, test_loader, loss):
    ...
    (
        ...
        decode_train_array,
        code_train_array,
        *optional_train_values,
    ) = dataloader_to_datasets(train_loader, auto_encoder)
    (
        data_val_array,
        labels_val_array,
        decode_val_array,
        code_val_array,
        *optional_val_values,
    ) = dataloader_to_datasets(val_loader, auto_encoder)
    (
        data_test_array,
        labels_test_array,
        decode_test_array,
        code_test_array,
        *optional_test_values,
    ) = dataloader_to_datasets(test_loader, auto_encoder)
