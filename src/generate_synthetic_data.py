...
def generate_matrices_block_diag(
    number_matrices, size_matrices, lower_bound=3, upper_bound=6
):
    ...
    grid = torch.arange(
        b_inf, b_sup, (b_sup - b_inf) / size_matrices
    )  # pas adapté pour être sur d'avoir une grid de size_matrices valeurs
    for _ in range(number_matrices):
        epsilon = torch.randn(size_matrices) * 0.5
        X_i = (
            U.T @ torch.diag(grid + epsilon) @ U
        )  # création matrice SDP en fonction de val propres grid+eps et matrice ortho U
        X_i = (X_i.T + X_i) * 0.5  # symetrie
        X_i_diag = block_diag(
            X_i.numpy(), X_i.numpy()
        )  # matrice carré avec deux carré qui sont X_i placés en diag et 0 sinon
        X_i_diag = X_i_diag / torch.linalg.matrix_norm(
            torch.from_numpy(X_i_diag)
        )  # normalisation
        X = torch.cat(
            (X, X_i_diag.unsqueeze(0)), dim=0
        )  # rajout de la matrice au dataset
...
def generate_synthetic_data_block_diag(
    number_matrices, size_matrices, batch_size, noise, std
):
    X = generate_matrices_block_diag(
        number_matrices=number_matrices, size_matrices=size_matrices
    )
    dataset, labels = generate_dataset_block_diag(X)
    return preprocess_data_cov(dataset, labels, batch_size, noise, std)


def generate_datasets_block_diag(
    number_dataset, number_matrices, size_matrices, batch_size, noise, std
):
    for _ in range(number_dataset):
        train_loader, val_loader, test_loader = generate_synthetic_data_block_diag(
        ...
        save_synthetic_data(
            ...
            val_loader,
            test_loader,
            name=c.parsing_synthetic_data_block_diag,
        )
...
def generate_matrices_lambda_mu(
    number_matrices, size_matrices, lambda_mean=3, mu_mean=6
):
    ...
    for _ in range(number_matrices):
        ...
        for _ in range(size_matrices):
            ...
            diag_lambda_flatten = torch.cat(
                (diag_lambda_flatten, value_lambda_diag.unsqueeze(0)), dim=0
            )
            diag_lambda = diag_lambda_flatten.squeeze(dim=1)
            value_mu_diag = torch.normal(mean=mu_mean, std=epsilon)
            diag_mu_flatten = torch.cat(
                (diag_mu_flatten, value_mu_diag.unsqueeze(0)), dim=0
            )
        ...
        sigma_plus_diag = torch.from_numpy(
            block_diag(X_i_lambda.numpy(), X_i_mu.numpy())
        )  # matrice carré avec deux carré en diag, 0 sinon
        X_sigma_plus = torch.cat((X_sigma_plus, sigma_plus_diag.unsqueeze(0)), dim=0)
        sigma_minus_diag = torch.from_numpy(
            block_diag(X_i_mu.numpy(), X_i_lambda.numpy())
        )  # matrice carré avec deux carré en diag, 0 sinon
        X_sigma_minus = torch.cat((X_sigma_minus, sigma_minus_diag.unsqueeze(0)), dim=0)
    return (
        X_sigma_plus.unsqueeze(dim=1).double(),
        X_sigma_minus.unsqueeze(dim=1).double(),
    )  # on rajoute le channel
...
def generate_dataset_lambda_mu(X_sigma_plus, X_sigma_minus):
    ...
    return torch.cat((X_sigma_plus, X_sigma_minus)), np.concatenate(
        (labels_sigma_plus, labels_sigma_minus)
    )


def generate_synthetic_data_lambda_mu(
    number_matrices, size_matrices, batch_size, noise, std
):
    X_sigma_plus, X_sigma_minus = generate_matrices_lambda_mu(
        number_matrices=number_matrices, size_matrices=size_matrices
    )
...
def generate_datasets_lambda_mu(
    number_dataset, number_matrices, size_matrices, batch_size, noise, std
):
    for _ in range(number_dataset):
        ...
        save_synthetic_data(
            ...
            val_loader,
            test_loader,
            name=c.parsing_synthetic_data_lambda_mu,
        )
...
def generate_dataset_geodesics(number_matrices, size_matrices, batch_size, noise, std):
    ...
    train_loader, val_loader, test_loader = preprocess_data_cov(
        data_tensor, labels, batch_size, noise, std
    )
    save_synthetic_data(
        train_loader, val_loader, test_loader, name=c.parsing_synthetic_data_geodesics
    )


def generate_datasets_geodesics(
    number_dataset, number_matrices, size_matrices, batch_size, noise, std
):
    for _ in range(number_dataset):
        generate_dataset_geodesics(
            number_matrices, size_matrices, batch_size, noise, std
        )
...
def main():
    ...
    if args.synthetic_generation == c.parsing_synthetic_data_lambda_mu:
        generate_datasets_lambda_mu(
            number_dataset=args.number_dataset,
            number_matrices=args.number_matrices,
            size_matrices=args.size_block_matrices,
            batch_size=args.batch_size,
            noise=args.noise,
            std=args.std,
        )
    elif args.synthetic_generation == c.parsing_synthetic_data_geodesics:
        generate_datasets_geodesics(
            number_dataset=args.number_dataset,
            number_matrices=args.number_matrices,
            size_matrices=args.size_block_matrices,
            batch_size=args.batch_size,
