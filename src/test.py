    ...
    for data in test_loader:
        with torch.no_grad():
            ...
            if isinstance(criterion, torch.nn.MSELoss):
                trustworthiness_recomp += trustworthiness(
                    data_test,
                    outputs_test,
                    pairwise_distance=pairwise_euclidean_distances,
                )
                if (
                    is_one_channel
                ):  # on peut pas si ho diff hi car on compare ho channels à hi
                    trustworthiness_encoding += trustworthiness(
                        data_test, z, pairwise_distance=pairwise_euclidean_distances
                    )
            else:
